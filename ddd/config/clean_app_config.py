...
class CleanAppConfig(AppConfig):
    ...
    def __load_custom_models_module(self):
        print('load module:::::', f"{self.name}.{self.CUSTOM_MODELS_MODULE}")
    ...
    def __load_custom_admin_module(self, admin_module: str, register_to: AdminSite):
        autodiscover_modules(admin_module, register_to=register_to)

    def __load_custom_migration_module(self, migration_module: str):
        print('load migrations:', f"{self.name}.{migration_module}")
